...
namespace PSXSharp.Core.x64_Recompiler {
    ...
    public static unsafe class x64_JIT {
        ...
        /*
            ...
            Label validateBlock = asm.CreateLabel();
            Label validateLoop = asm.CreateLabel();
            ...
            asm.cmp(__dword_ptr[rax + 4], 1);           //Read "IsCompiled"
            asm.je(validateBlock);
            ...
            //Invalidation
            //We compute checksum (addition of all instructions) and compare it with the stored value
            //TODO: Implement SIMD/AVX Path

            asm.Label(ref validateBlock);

            asm.mov(rax, (ulong)CPU_x64_Recompiler.BUS.RAM.GetMemoryPointer());     //Get memory pointer
            asm.mov(r8d, __dword_ptr[rbx + 0]);                                     //Get block address
            asm.and(r8d, 0x1FFFFFFF);                                               //Mask block address
            asm.add(rax, r8);                                                       //Add block address to base pointer

            asm.mov(ecx, __dword_ptr[rbx + 8]);                                     //Get total MIPS instructions in ecx
            asm.dec(ecx);                                                           //Point to the last instruction
            asm.xor(rdx, rdx);                                                      //Clear rdx

            //Loop ecx times
            asm.Label(ref validateLoop);
            asm.add(edx, __dword_ptr[rax + rcx * 4]);
            asm.dec(ecx);
            asm.jns(validateLoop);                                                 

            asm.cmp(edx, __dword_ptr[rbx + 16]);                                    //Compare with MIPS_Checksum
            asm.jne(recompile);
            asm.jmp(executeBlock);
            ...
            asm.mov(r15, GetRecompileBlockAddress());
            asm.call(r15);
            //asm.jmp(executeBlock); //Not needed
