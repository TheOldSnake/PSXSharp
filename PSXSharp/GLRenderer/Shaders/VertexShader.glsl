...
// Adjusts the quad corner position to correct aspect ratio.
// The same offset is symmetrically applied on each axis, shrinking or expanding
// left/right (X) and up/down (Y) equally so the quad remains centered.
vec4 handleAspectRatio(int id) {
   const vec2 xy[4] = vec2[4](
        vec2(-1,  1),
        vec2( 1,  1),
        vec2(-1, -1),
        vec2( 1, -1)
